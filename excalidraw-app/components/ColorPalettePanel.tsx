 ...
 *
 * Photoshop-style color picker popup.
 * Opens near the color icon, stays on screen.
 */
...
export const ColorPalettePanel: React.FC<ColorPalettePanelProps> = ({
  ...
  useEffect(() => {
    if (isOpen && panelRef.current) {
      ...
      let x = 100; // Near left sidebar
      let y = viewportHeight / 2 - rect.height / 2; // Vertically centered

      // Keep within viewport
      if (x + rect.width > viewportWidth - 20) {
        x = viewportWidth - rect.width - 20;
      }
      if (y < 60) y = 60; // Below top bar
