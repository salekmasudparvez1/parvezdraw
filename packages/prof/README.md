...
```bash
npm install react react-dom @prof/core
# or
yarn add react react-dom @prof/core
```

> **Note**: If you want to try unreleased changes, use `@prof/core@next`.
...
- If imports or entrypoints are unclear, inspect `node_modules/@prof/core/package.json`. The installed package exports are the source of truth.
...
## Migrating to `@prof/core@0.18.x`

Version `0.18.x` removes the old `types/`-prefixed deep import paths. If you were importing types from `@prof/core/types/...`, switch to the new type-only subpaths below.

| Old path | New path |
| --- | --- |
| `@prof/core/types/data/transform.js` | `@prof/core/element/transform` |
| `@prof/core/types/data/types.js` | `@prof/core/data/types` |
| `@prof/core/types/element/types.js` | `@prof/core/element/types` |
| `@prof/core/types/utility-types.js` | `@prof/core/common/utility-types` |
| `@prof/core/types/types.js` | `@prof/core/types` |

Drop the `.js` extension. The new package `exports` map resolves these paths without it.

These deep subpaths are for `import type` only. Runtime imports should come from the package root, plus `@prof/core/index.css` for styles.
...
By default, Excalidraw downloads the fonts it needs from the [CDN](https://esm.run/@prof/core/dist/prod).

For self-hosting, copy the contents of `node_modules/@prof/core/dist/prod/fonts` into the path where your app serves static assets, for example `public/`. Then set `window.EXCALIDRAW_ASSET_PATH` to that same path:
...
Read the [integration docs](https://docs.excalidraw.com/docs/@prof/core/integration).

## API

Read the [API docs](https://docs.excalidraw.com/docs/@prof/core/api).

## Contributing

Read the [contributing docs](https://docs.excalidraw.com/docs/@prof/core/contributing).
